...
- `YYYY-MM-DD HH:MM` — 日期 + 时间（如 `2026-06-22 14:00`）
- `today HH:MM` — 相对日期 + 时间（如 `today 15:00`）
...
# 创建带时间的待办
timeline-cli todo add "代码评审" --at "2026-06-22 14:00"
...
# 创建无日期待办
timeline-cli todo add "有空时整理桌面" --at ""
...
参数：
- `--range RANGE` — 必填，日期范围（见「`--range` 参数」章节）
...
# 记录过去的事件
timeline-cli event add "完成报告" --at "yesterday 15:00"
timeline-cli event add "提交代码" --at "-2h"

# 记录带详情的事件
timeline-cli event add "发布版本" --at "2026-06-21 10:00" --detail "v1.2.0"
```
